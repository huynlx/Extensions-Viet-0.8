...
var _Sources = (() => {
  ...
  var HentaiVN_exports = {};
  __export(HentaiVN_exports, {
    HentaiVN: () => HentaiVN,
    HentaiVNInfo: () => HentaiVNInfo
  });
  ...
  // src/HentaiVN/HentaiVNParser.ts
  var Parser = class {
    ...
    parseRandomSection($) {
      ...
      $("ul.page-random li").each((_, element) => {
        ...
        const lastChapter = $item.find(".meta").text().trim() || void 0;
        if (mangaId && title && !mangaId.includes("javascript")) {
          mangaList.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parseMangaDetails($, mangaId) {
      ...
      const getInfoText = (label) => {
        return $pageInfo.find("p").filter((_, el) => $(el).find("span.info").text().includes(label)).find("span").last().text().trim();
      };
      const authorVal = getInfoText("T\xE1c gi\u1EA3");
      const author = authorVal && authorVal !== "\u0110ang c\u1EADp nh\u1EADt" ? authorVal : "\u0110ang c\u1EADp nh\u1EADt";
      const statusVal = getInfoText("T\xECnh Tr\u1EA1ng");
      const status = statusVal.includes("\u0110\xE3 ho\xE0n th\xE0nh") ? "Completed" : "Ongoing";
      const arrayTags = [];
      ...
      const views = $pageInfo.find("span.info").filter((_, el) => $(el).text().includes("L\u01B0\u1EE3t xem")).next("span").text().trim();
      const rawDesc = $pageInfo.find('p[style*="white-space:pre-wrap"]').text().trim();
      const descParts = [];
      if (altName) descParts.push(`T\xEAn kh\xE1c: ${altName}`);
...
${rawDesc}`);
    ...
    parseTags($) {
      ...
      const sortOptions = [
        ...
        { id: "sort=least-viewed", label: "Xem \xEDt nh\u1EA5t" }
      ];
  ...
  // src/HentaiVN/HentaiVNSetting.ts
  var DEFAULT_BASE_URL = "https://hentaivnreal.com";
  ...
  return __toCommonJS(HentaiVN_exports);
})();
