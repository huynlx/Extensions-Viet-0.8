...
var _Sources = (() => {
  ...
  // src/SayHentai/SayHentaiParser.ts
  var Parser = class {
    ...
    parseNewUpdatedSection($) {
      ...
      $(".page-item-detail").each((_, element) => {
        ...
        const subtitle = item.find(".list-chapter .chapter-item .chapter a").first().text().trim() || void 0;
        if (title) {
          mangaList.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parsePopularSection($) {
      ...
      $("li.popular-item-wrap").each((_, element) => {
        ...
        const mangaId = href.split("/").pop()?.split("?")[0] ?? "";
        const title = $item.find("h3.widget-title a").text().trim() || $item.find("img").attr("alt")?.replace(/^Ảnh bìa truyện\s*/i, "").trim() || "";
        ...
        const subtitle = views || postOn || void 0;
        if (mangaId && title && !mangaId.includes("javascript")) {
          mangaList.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parseMangaDetails($, mangaId) {
      ...
      const getInfoTextByHeading = (headingText) => {
        let result = "";
      ...
      const authorVal = getInfoTextByHeading("T\xE1c gi\u1EA3");
      const author = authorVal && authorVal !== "" ? authorVal : "\u0110ang c\u1EADp nh\u1EADt";
      const nhomDich = getInfoTextByHeading("Nh\xF3m d\u1ECBch");
      ...
      let ratingStr = "";
      if (ratingScore) {
        ratingStr = ratingCount ? `${ratingScore}/5 (${ratingCount} b\xECnh ch\u1ECDn)` : `${ratingScore}/5`;
      }
...
\u2B50 Rating: ${ratingStr}`);
      if (nhomDich) descParts.push(`\u{1F6A9} Nh\xF3m d\u1ECBch: ${nhomDich}`);
      if (views) descParts.push(`\u{1F441} View: ${views}`);
      if (lastUpdateStr) descParts.push(`\u{1F552} C\u1EADp nh\u1EADt: ${lastUpdateStr}`);
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          hentai: true,
          rating: ratingScore ? parseFloat(ratingScore) : void 0
        })
      });
    }
    ...
    parseChapterListFromSelect($, params) {
      const chapters = [];
  ...
  // src/SayHentai/SayHentaiSetting.ts
  var DEFAULT_BASE_URL = "https://sayhentai.cx";
  ...
  var SayHentai = class {
    ...
    async fetchMangaPageCached(mangaId) {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const sectionConfig = {
        new_updated: {
          ...
          parse: ($2) => this.parser.parseNewUpdatedSection($2)
        },
        done: {
