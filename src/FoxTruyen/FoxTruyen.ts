...
import { isLastPage, Parser } from './FoxTruyenParser';
import { domainSettings, getDomain, resetSettings } from './FoxTruyenSetting';

const DEFAULT_DOMAIN = 'https://foxtruyen2.com';

export const FoxTruyenInfo: SourceInfo = {
    version: '1.0.0',
    name: 'FoxTruyen',
    icon: 'icon.png',
...
export class FoxTruyen implements SearchResultsProviding, MangaProviding, ChapterProviding, HomePageSectionsProviding {
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        const sections: HomeSection[] = [
            ...
            App.createHomeSection({ id: 'vote', title: 'Top Bình Chọn', containsMoreItems: false, type: HomeSectionType.singleRowLarge }),
            App.createHomeSection({ id: 'view', title: 'Top Tuần', containsMoreItems: false, type: HomeSectionType.singleRowLarge }),
            App.createHomeSection({ id: 'new_added', title: 'Truyện Mới', containsMoreItems: true, type: HomeSectionType.singleRowNormal }),
            App.createHomeSection({ id: 'full', title: 'Truyện Full', containsMoreItems: true, type: HomeSectionType.singleRowNormal }),
        ];

        // 2. Map URL tương ứng cho từng Section ID
        ...
        // 3. Emit tất cả section rỗng ngay lập tức để UI dựng Skeleton Layout
        for (const section of sections) {
            sectionCallback(section);
        }

        // 4. Tạo trước các Promise duy nhất cho từng URL độc nhất (Tránh Race Condition)
        ...
        // 5. Mapping parser riêng cho các section
        const sectionParsers: Record<string, ($: CheerioAPI) => PartialSourceManga[]> = {
            featured: ($) => this.parser.parseFeaturedSection($),
            vote: ($) => this.parser.parseExtraSection($, 'Bình Chọn'),
            view: ($) => this.parser.parseExtraSection($, 'Xem Nhiều'),
        };

        // 6. Chạy bóc tách dữ liệu song song cho từng section mà không lo request trùng
    ...
    async CloudFlareError(status: number): Promise<void> {
        if (status === 503 || status === 403) {
            const baseUrl = await this.getBaseUrl();
            throw new Error(`CLOUDFLARE BYPASS ERROR:\nPlease go to home page ${FoxTruyen.name} source (${baseUrl}) and press the cloud icon.`);
        }
    }
