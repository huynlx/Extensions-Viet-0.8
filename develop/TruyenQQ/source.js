...
var _Sources = (() => {
  ...
  var Parser = class {
    convertTime(timeAgo) {
      ...
      if (timeAgo.includes("gi\xE2y") || timeAgo.includes("secs")) {
        ...
        const finalD = `${fixD[1]}/${fixD[0]}/${(/* @__PURE__ */ new Date()).getFullYear()}`;
        return /* @__PURE__ */ new Date(`${finalD} ${H}`);
      } else {
        const split = timeAgo.split("/");
        return /* @__PURE__ */ new Date(`${split[1]}/${split[0]}/${split[2]}`);
      }
    }
    ...
    parseMangaDetails($, mangaId) {
      ...
      const desc = decodeHTML(
        $("div.detail-content > p").map((_, el) => $(el).text().trim()).get().filter((text) => text.length > 0).join("\n\n")
      );
    ...
    parseTags($) {
      ...
      const arrayRankings = [];
      $("p.mega_menu").filter((_, el) => $(el).text().includes("X\u1EBFp H\u1EA1ng")).siblings(".hidden_menu.book_tags").find(".book_tags_content p a").each((_, el) => {
      ...
      const parseSelectOptions = (selector, prefix) => {
        ...
        $(selector).find("option").each((_, el) => {
          ...
          if (value === void 0 || value === "" || value === "-1" || prefix === "country" && value === "0" || !label) {
            return;
          }
          tags.push(
            App.createTag({
  ...
  // src/TruyenQQ/TruyenQQSetting.ts
  var DEFAULT_BASE_URL = "https://truyenqqko.com";
  ...
  // src/TruyenQQ/TruyenQQ.ts
  var DEFAULT_DOMAIN = "https://truyenqqko.com";
  var TruyenQQInfo = {
    version: "1.0.0",
    name: "TruyenQQ",
    icon: "icon.png",
  ...
  var TruyenQQ = class _TruyenQQ {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const sections = [
        ...
        App.createHomeSection({ id: "hot", title: "\u0110\u1ED9c Quy\u1EC1n Truy\u1EC7n QQ", containsMoreItems: false, type: import_types.HomeSectionType.singleRowLarge }),
        App.createHomeSection({ id: "new_updated", title: "Truy\u1EC7n M\u1EDBi C\u1EADp Nh\u1EADt", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal }),
        App.createHomeSection({ id: "favorite", title: "Truy\u1EC7n Y\xEAu Th\xEDch", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal }),
        App.createHomeSection({ id: "new_added", title: "Truy\u1EC7n M\u1EDBi Th\xEAm G\u1EA7n \u0110\xE2y", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal }),
        App.createHomeSection({ id: "full", title: "Truy\u1EC7n \u0110\xE3 Ho\xE0n Th\xE0nh", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal })
      ];
      const urlMap = {
...
Please go to home page ${_TruyenQQ.name} source (${baseUrl}) and press the cloud icon.`);
      }
    }
