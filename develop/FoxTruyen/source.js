...
var _Sources = (() => {
  ...
  // src/FoxTruyen/FoxTruyen.ts
  var FoxTruyen_exports = {};
  __export(FoxTruyen_exports, {
    FoxTruyen: () => FoxTruyen,
    FoxTruyenInfo: () => FoxTruyenInfo
  });
  ...
  var parseDate = (dateString) => {
    ...
    if (str.includes("tr\u01B0\u1EDBc") || str.includes("v\u1EEBa xong") || str.includes("v\xE0i gi\xE2y")) {
      const numberMatch = str.match(/\d+/);
      const amount = numberMatch?.[0] ? parseInt(numberMatch[0], 10) : 1;
      if (str.includes("gi\xE2y") || str.includes("v\u1EEBa xong")) {
        now.setSeconds(now.getSeconds() - amount);
        return now;
      }
      if (str.includes("ph\xFAt")) {
  ...
  // src/FoxTruyen/FoxTruyenParser.ts
  var Parser = class {
    ...
    parseHotSection($) {
      const featuredItems = [];
      $("#div_qq .list_grid li").each((_, manga) => {
    ...
    parseMangaDetails($, mangaId) {
      ...
      const author = decodeHTML($('.fx-meta__row:contains("T\xE1c gi\u1EA3") .fx-meta__val').text().trim() || "\u0110ang C\u1EADp Nh\u1EADt");
      const artist = author;
      ...
      const statsLines = [
        views ? `\u{1F441}\uFE0F L\u01B0\u1EE3t xem: ${views}` : "",
        follows ? `\u{1F516} Theo d\xF5i: ${follows}` : "",
        comments ? `\u{1F4AC} B\xECnh lu\u1EADn: ${comments}` : "",
        lastUpdate ? `\u{1F552} C\u1EADp nh\u1EADt: ${lastUpdate}` : ""
      ].filter(Boolean).join("\n");
      ...
      if (!rawDesc) {
        rawDesc = $synopsis.text().replace(/^[\s:]+/, "").trim();
      }
...
${statsLines}`.trim() : rawDesc);
    ...
    parseTags($) {
      ...
      const arrayRankings = [
        App.createTag({ id: "ranking.top-ngay", label: decodeHTML("Top Ng\xE0y") }),
        App.createTag({ id: "ranking.top-tuan", label: decodeHTML("Top Tu\u1EA7n") }),
        App.createTag({ id: "ranking.top-thang", label: decodeHTML("Top Th\xE1ng") }),
        App.createTag({ id: "ranking.top-binh-chon", label: decodeHTML("Top B\xECnh Ch\u1ECDn") })
      ];
      const parseSelectOptions = (selector, prefix) => {
        const tags = [];
      ...
      return [
        ...
        App.createTagSection({
          id: "3",
          label: decodeHTML("Tr\u1EA1ng Th\xE1i (Ch\u1EC9 ch\u1ECDn 1)"),
  ...
  // src/FoxTruyen/FoxTruyenSetting.ts
  var DEFAULT_BASE_URL = "https://foxtruyen2.com";
  ...
  // src/FoxTruyen/FoxTruyen.ts
  var DEFAULT_DOMAIN = "https://foxtruyen2.com";
  var FoxTruyenInfo = {
    version: "1.0.0",
    name: "FoxTruyen",
    icon: "icon.png",
    ...
    description: "Extension that pulls manga from FoxTruyen.",
  ...
  var FoxTruyen = class _FoxTruyen {
    ...
    async getHomePageSections(sectionCallback) {
      const baseUrl = await this.getBaseUrl();
      const sections = [
        App.createHomeSection({ id: "featured", title: "Truy\u1EC7n \u0110\u1EC1 C\u1EED", containsMoreItems: false, type: import_types.HomeSectionType.featured }),
        ...
        App.createHomeSection({ id: "vote", title: "Top B\xECnh Ch\u1ECDn", containsMoreItems: false, type: import_types.HomeSectionType.singleRowLarge }),
        App.createHomeSection({ id: "view", title: "Top Tu\u1EA7n", containsMoreItems: false, type: import_types.HomeSectionType.singleRowLarge }),
        App.createHomeSection({ id: "new_added", title: "Truy\u1EC7n M\u1EDBi", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal }),
        App.createHomeSection({ id: "full", title: "Truy\u1EC7n Full", containsMoreItems: true, type: import_types.HomeSectionType.singleRowNormal })
      ];
      const urlMap = {
        featured: `${baseUrl}`,
        ...
        vote: `${baseUrl}`,
        view: `${baseUrl}`,
        new_added: `${baseUrl}/truyen-moi`,
        full: `${baseUrl}/truyen-hoan-thanh`
      };
      for (const section of sections) {
        sectionCallback(section);
      }
      const urlPromiseMap = /* @__PURE__ */ new Map();
      const uniqueUrls = Array.from(new Set(Object.values(urlMap)));
      for (const url of uniqueUrls) {
        urlPromiseMap.set(url, this.DOMHTML(url));
      }
      const sectionParsers = {
        featured: ($) => this.parser.parseFeaturedSection($),
        vote: ($) => this.parser.parseExtraSection($, "B\xECnh Ch\u1ECDn"),
        view: ($) => this.parser.parseExtraSection($, "Xem Nhi\u1EC1u")
      };
      const fetchPromises = sections.map(async (section) => {
        ...
        try {
          ...
          const parseFn = sectionParsers[section.id] ?? (($2) => this.parser.parseSearchResults($2));
    ...
    async getSearchResults(query, metadata) {
      ...
      if (rankingPath) {
        ...
        param = `?q=${encodeURIComponent(query.title ?? "")}&category=${search.genres}${paramExgenres}&country=${search.country}&status=${search.status}&minchapter=${search.minchapter}&sort=${search.sort}`;
      }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      metadata = isLastPage($) ? void 0 : { page: page + 1 };
      return App.createPagedResults({
        results: manga,
        metadata
      });
...
Please go to home page ${_FoxTruyen.name} source (${baseUrl}) and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(FoxTruyen_exports);
})();
