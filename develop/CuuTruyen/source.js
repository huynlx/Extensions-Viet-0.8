...
var _Sources = (() => {
  ...
  // src/CuuTruyen/includes/genreTags.json
  var genreTags_default = [
    ...
    {
      label: "du h\xE0nh th\u1EDDi gian",
      id: "du-hanh-thoi-gian"
    },
    {
      label: "\u0111\xE3 c\xF3 d\u1EA5u r\u0103ng c\u1EE7a assmin c\u1EE9u truy\u1EC7n",
      id: "da-co-dau-rang-cua-assmin-cuu-truyen"
    },
    {
    ...
    {
      label: "t\xECnh y\xEAu",
      id: "tinh-yeu"
    },
    {
      label: "t\xECnh y\xEAu thu\u1EA7n khi\u1EBFt",
      id: "tinh-yeu-thuan-khiet"
    },
    {
      label: "to\xE1n h\u1ECDc",
      id: "toan-hoc"
  ...
  var Parser = class {
    ...
    parseMangaDetails($, mangaId) {
      ...
      const author = decodeHTML(
        $container.find('a[href*="/tac-gia/"]').map((_, el) => $(el).text().trim()).get().join(", ") || "Ch\u01B0a r\xF5"
      );
      const artist = decodeHTML(
        $container.find('a[href*="/nhom-dich/"]').map((_, el) => $(el).text().trim()).get().join(", ")
      );
      ...
      const rawDesc = $container.find("div.mg-plot p").map((_, el) => $(el).text().trim()).get().filter((text) => text.length > 0 && text !== "T\xF3m t\u1EAFt").join("\n\n");
      const desc = decodeHTML(rawDesc);
      const status = decodeHTML($container.find('a[href*="filter%5Bstatus%5D"], a[href*="filter[status]"]').text().trim() || "\u0110ang ti\u1EBFn h\xE0nh");
    ...
    parseChapterList($) {
      ...
      $elements.each((idx, obj) => {
        ...
        const name = decodeHTML($item.find("div.grow span").text().trim());
        ...
        const group = views ? decodeHTML(`${views} l\u01B0\u1EE3t xem`) : void 0;
        if (id && name) {
          chapters.push(
            App.createChapter({
              id,
    ...
    parseChapterDetails($) {
      const pages = [];
      $("div.text-center img").each((_, obj) => {
        const $img = $(obj);
  ...
  // src/CuuTruyen/CuuTruyenSetting.ts
  var DEFAULT_BASE_URL = "https://cuutruyen.moe";
  ...
  var CuuTruyen = class _CuuTruyen {
...
Please go to home page ${_CuuTruyen.name} source (${baseUrl}) and press the cloud icon.`);
      }
    }
