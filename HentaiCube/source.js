...
var _Sources = (() => {
  ...
  // src/HentaiCube/HentaiCube.ts
  var HentaiCube_exports = {};
  __export(HentaiCube_exports, {
    HentaiCube: () => HentaiCube,
    HentaiCubeInfo: () => HentaiCubeInfo
  });
  ...
  // src/HentaiCube/HentaiCubeParser.ts
  var Parser = class {
    parseFeaturedSection($) {
      const mangaList = [];
      const addedMangaIds = /* @__PURE__ */ new Set();
    ...
    parseNewUpdatedSection($) {
      ...
      $(".page-listing-item .page-item-detail").each((_, element) => {
        ...
        const lastChapter = $item.find(".list-chapter .chapter-item .chapter a").first().text().replace(/\s+/g, " ").trim() || void 0;
    ...
    parseLoopResults($) {
      ...
      $(".row.c-tabs-item__content").each((_, element) => {
        ...
        const lastChapter = $item.find(".tab-meta .chapter a").first().text().replace(/\s+/g, " ").trim() || void 0;
        if (mangaId && title && !mangaId.includes("javascript")) {
          mangaList.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parseMangaDetails($, mangaId) {
      ...
      const authorVal = getSummaryContent("T\xE1c gi\u1EA3");
      const author = authorVal && authorVal !== "\u0110ang c\u1EADp nh\u1EADt" ? authorVal : "\u0110ang c\u1EADp nh\u1EADt";
      const statusVal = getSummaryContent("T\xECnh tr\u1EA1ng");
      const status = statusVal.toLowerCase().includes("ho\xE0n th\xE0nh") ? "Completed" : "Ongoing";
      const arrayTags = [];
      ...
      const rawDesc = $summary.text().replace(/[ \t]+/g, " ").replace(/\n\s*\n/g, "\n\n").trim();
      const descParts = [];
      if (altName) descParts.push(`T\xEAn kh\xE1c: ${decodeHTML(altName)}`);
      let timeAndView = "";
      if (postTime) timeAndView += `\u23F0 ${decodeHTML(postTime)}`;
      if (views) {
        timeAndView += (timeAndView ? "\n" : "") + `\u{1F441} L\u01B0\u1EE3t xem: ${decodeHTML(views)}`;
      }
    ...
    parseChapterListFromArray(chapterElements, cheerioInstance) {
      const chapters = [];
  ...
  // src/HentaiCube/HentaiCubeSetting.ts
  var DEFAULT_BASE_URL = "https://hentaicube.xyz";
  ...
  // src/HentaiCube/HentaiCube.ts
  var DOMAIN = "https://hentaicube.xyz";
  var HentaiCubeInfo = {
    version: "1.0.0",
    name: "HentaiCube",
    icon: "icon.png",
  ...
  var HentaiCube = class {
    ...
    async getHomePageSections(sectionCallback) {
      const baseUrl = await this.getBaseUrl();
      const featuredSection = App.createHomeSection({ id: "featured", title: "\u0110\u1EC1 C\u1EED", type: import_types.HomeSectionType.featured, containsMoreItems: false });
      const hotSection = App.createHomeSection({ id: "hot", title: "Trending", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const newUpdatedSection = App.createHomeSection({ id: "new_updated", title: "V\u1EEBa C\u1EADp Nh\u1EADt", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const viewSection = App.createHomeSection({ id: "view", title: "\u0110\u1ECDc Nhi\u1EC1u Nh\u1EA5t", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const newSection = App.createHomeSection({ id: "new", title: "M\u1EDBi", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const randomSection = App.createHomeSection({ id: "random", title: "Ng\u1EABu Nhi\xEAn", type: import_types.HomeSectionType.singleRowLarge, containsMoreItems: false });
      const doneSection = App.createHomeSection({ id: "done", title: "Ho\xE0n Th\xE0nh", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      if (cached && now - cached.timestamp < this.CHAPTER_DETAIL_CACHE_TTL) {
        ...
        if (!currentToken) {
          throw new Error(`Kh\xF4ng t\xECm th\u1EA5y token cho chapter: ${chapterId}`);
        }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const sectionConfig = {
        new_updated: {
          ...
          parse: ($2) => this.parser.parseSearchResults($2)
        },
        hot: {
        ...
        new: {
          ...
          parse: ($2) => this.parser.parseLoopResults($2)
        }
      };
  ...
  return __toCommonJS(HentaiCube_exports);
})();
