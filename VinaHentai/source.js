...
var _Sources = (() => {
  ...
  // src/VinaHentai/VinaHentai.ts
  var VinaHentai_exports = {};
  __export(VinaHentai_exports, {
    VinaHentai: () => VinaHentai,
    VinaHentaiInfo: () => VinaHentaiInfo
  });
  ...
  // src/VinaHentai/VinaHentaiParser.ts
  var VinaHentaiParser = class {
    constructor() {
      this.imageUrlRegex = /https:\/\/[^"'\s\\]+\/manga-images\/[^"'\s\\]+/g;
      this.pageRegex = /page=(\d+)/;
      this.numberRegex = /\d+/;
    }
    // ============================ MANGA LIST / SEARCH ============================
    parseAllTimeSection($) {
      ...
      $('a[href^="/truyen-hentai/"]').each((_, element) => {
        ...
        let subtitle = views ? views : void 0;
        if (rank) {
    ...
    parseFeaturedSection($) {
      ...
      $(".banner-desktop-card").each((_, element) => {
        ...
        const subtitle = subtitleAttr ? decodeHTML(subtitleAttr) : void 0;
        if (mangaId && title) {
          items.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parseMangaList($) {
      ...
      $('.grid a[href*="/truyen-hentai/"]').each((_, el) => {
        ...
        const subtitle = subtitleText ? decodeHTML(subtitleText) : void 0;
        if (mangaId && title) {
          mangas.push(
            App.createPartialSourceManga({
              mangaId,
    ...
    parseHasNextPage($, currentMangaCount, currentUrl, isSearch) {
      if (!isSearch) {
    ...
    parseMangaDetails($, mangaId) {
      ...
      const author = decodeHTML(
        $('a[href*="/authors/"]').map((_, el) => $(el).text().trim()).get().filter((text) => text && !text.startsWith("+")).join(", ") || "Ch\u01B0a r\xF5"
      );
      const artist = "";
      let image = $('img[alt*="B\xECa"]').attr("src") || $('img[src*="story-images"]').attr("src") || "";
      if (image.startsWith("//")) {
        image = `https:${image}`;
      }
      const desc = decodeHTML($("#manga-description-section .text-txt-secondary").text().trim());
      const bodyText = $("body").text();
      let status = "\u0110ang ti\u1EBFn h\xE0nh";
      if (bodyText.includes("\u0110\xE3 ho\xE0n th\xE0nh")) {
        status = "\u0110\xE3 ho\xE0n th\xE0nh";
      } else if (bodyText.includes("\u0110ang ti\u1EBFn h\xE0nh")) {
        status = "\u0110ang ti\u1EBFn h\xE0nh";
      }
    ...
    parseChapterList($) {
      ...
      elements.forEach((el, idx) => {
        ...
        const group = views ? `${views} l\u01B0\u1EE3t xem` : void 0;
        rawChapters.push(
          App.createChapter({
            id: chapterId,
            name,
    ...
    parseGenres($) {
      ...
      const sortTags = [
        ...
        App.createTag({ id: "likes", label: decodeHTML("\u0110\xE1nh gi\xE1 cao") }),
        App.createTag({ id: "oldest", label: decodeHTML("C\u0169 nh\u1EA5t") })
      ];
      const statusTags = [
        App.createTag({ id: "", label: decodeHTML("T\u1EA5t c\u1EA3") }),
        App.createTag({ id: "ongoing", label: decodeHTML("\u0110ang ti\u1EBFn h\xE0nh") }),
        App.createTag({ id: "completed", label: decodeHTML("\u0110\xE3 ho\xE0n th\xE0nh") })
      ];
      return [
        ...
        App.createTagSection({
          id: "status",
          label: decodeHTML("T\xECnh tr\u1EA1ng"),
          tags: statusTags
        })
      ];
    }
    parseRelativeDate(dateStr) {
      ...
      if (dateStr.includes("gi\xE2y")) now.setSeconds(now.getSeconds() - num);
      else if (dateStr.includes("ph\xFAt")) now.setMinutes(now.getMinutes() - num);
      else if (dateStr.includes("gi\u1EDD")) now.setHours(now.getHours() - num);
      else if (dateStr.includes("ng\xE0y")) now.setDate(now.getDate() - num);
      else if (dateStr.includes("tu\u1EA7n")) now.setDate(now.getDate() - num * 7);
      else if (dateStr.includes("th\xE1ng")) now.setMonth(now.getMonth() - num);
  ...
  function isLastPageAllTime($) {
    const $pagination = $('nav[aria-label="Ph\xE2n trang manga m\u1ECDi th\u1EDDi \u0111\u1EA1i"]');
  ...
  function isLastPageSearch($) {
    ...
    const isDisabled = $lastButton.attr("disabled") !== void 0 || $lastButton.hasClass("cursor-not-allowed");
    if (isDisabled) {
      return true;
    }
  ...
  // src/VinaHentai/VinaHentaiSetting.ts
  var DEFAULT_BASE_URL = "https://vinahentai.blog";
  ...
  // src/VinaHentai/VinaHentai.ts
  var DEFAULT_DOMAIN = "https://vinahentai.blog";
  var VinaHentaiInfo = {
    version: "1.0.0",
    name: "VinaHentai",
    icon: "icon.png",
  ...
  var VinaHentai = class _VinaHentai {
    // 1 ngày cho chapter details
    constructor(cheerio) {
      this.cheerio = cheerio;
      this.stateManager = App.createSourceStateManager();
      this.parser = new VinaHentaiParser();
    ...
    async getSearchResults(query, metadata) {
      ...
      if (keyword) {
        ...
        const cacheKey2 = `search-${url}`;
        const now2 = Date.now();
        const cached2 = this.cache.get(cacheKey2);
        let $2;
        if (cached2 && now2 - cached2.timestamp < this.CACHE_TTL) {
          $2 = cached2.data;
        } else {
          $2 = await this.DOMHTML(url);
          this.cache.set(cacheKey2, { data: $2, timestamp: now2 });
        }
        const tiles2 = this.parser.parseSearchManga($2);
        const hasNextPage = this.parser.parseHasNextPage($2, tiles2.length, url, true);
        return App.createPagedResults({
          results: tiles2,
          metadata: hasNextPage ? { page: page + 1 } : void 0
        });
      }
      let genreSlug = void 0;
      let sort = "updatedAt";
      let status = "";
...
Please go to home page ${_VinaHentai.name} source (${baseUrl}) and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(VinaHentai_exports);
})();
