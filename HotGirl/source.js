...
var _Sources = (() => {
  ...
  // src/HotGirl/HotGirl.ts
  var HotGirl_exports = {};
  __export(HotGirl_exports, {
    HotGirl: () => HotGirl,
    HotGirlInfo: () => HotGirlInfo
  });
  ...
  var Parser = class {
    ...
    parseMangaDetails($, compositeId) {
      ...
      const descParts = [];
      if (views !== void 0) {
        descParts.push(`\u{1F441}\uFE0F ${decodeHTML(viewsText)}`);
      }
        ...
        if (chapterId) {
          ...
          chapters.push(
            App.createChapter({
              ...
              // Tên chapter (name) được chuẩn hóa và giải mã HTML entity
              chapNum,
      ...
      $("div.tagcloud a.tag-cloud-link").each((_, el) => {
        ...
        if (id && label) {
          tags.push(
            App.createTag({
              id,
              label: decodeHTML(`${label} ${countText}`.trim())
              // Nhãn thẻ (label) được chuẩn hóa và giải mã an toàn
            })
          );
        }
      });
  ...
  // src/HotGirl/HotGirlSetting.ts
  var DEFAULT_BASE_URL = "https://hotgirl.biz";
  ...
  return __toCommonJS(HotGirl_exports);
})();
