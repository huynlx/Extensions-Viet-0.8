...
import { extractChapterParams, LXChapterResponse, Parser } from './LXMangaParser';
import { domainSettings, getDomain, resetSettings } from './LXMangaSetting';

const DOMAIN = 'https://lxmanga.org';
...
export const LXMangaInfo: SourceInfo = {
    ...
    description: 'Extension that pulls manga from LXManga.',
...
export class LXManga implements SearchResultsProviding, MangaProviding, ChapterProviding, HomePageSectionsProviding {
    ...
    private async fetchMangaPage(mangaId: string): Promise<CheerioAPI> {
        ...
        const baseUrl = await this.getBaseUrl();
        const promise = this.DOMHTML(`${baseUrl}/${mangaId}`);
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        // 1. Tải trang chi tiết
        const request = App.createRequest({
            url: `https://lxmanga.org/${mangaId}`,
            method: 'GET',
        });
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data as string);

        // 2. Lấy postId và nonce
        const { postId, nonce } = extractChapterParams($);

        if (!postId || !nonce) {
            return [];
        }

        // 3. Gọi AJAX lấy full danh sách chapter
        const ajaxRequest = App.createRequest({
            url: 'https://lxmanga.org/wp-admin/admin-ajax.php',
            method: 'POST',
            headers: {
                'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
                'x-requested-with': 'XMLHttpRequest',
                referer: `https://lxmanga.org/${mangaId}`,
            },
            data: `action=baka_ajax&type=get_chapters_list&id=${postId}&chap=0&chapters_list_nonce=${nonce}`,
        });

        const ajaxResponse = await this.requestManager.schedule(ajaxRequest, 1);
        const jsonData: LXChapterResponse = typeof ajaxResponse.data === 'string' ? JSON.parse(ajaxResponse.data) : ajaxResponse.data;

        // 4. Trả về danh sách chapter chuẩn
        return this.parser.parseChapterListFromJSON(jsonData);
    }
