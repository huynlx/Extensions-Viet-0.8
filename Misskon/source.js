...
var _Sources = (() => {
  ...
  // src/Misskon/Misskon.ts
  var Misskon_exports = {};
  __export(Misskon_exports, {
    Misskon: () => Misskon,
    MisskonInfo: () => MisskonInfo
  });
  ...
  var Parser = class {
    parseNewUpdatedSection($) {
      ...
      $(".post-listing article.item-list").each((_, element) => {
        ...
        const subtitle = subtitleMatch?.[1]?.trim() ? decodeHTML(subtitleMatch[1].trim()) : void 0;
    ...
    parseMangaDetails($, compositeId) {
      ...
      const views = viewsMatch?.[1] ? parseInt(viewsMatch[1].replace(/[,.]/g, ""), 10) : void 0;
      let author = "MissKON";
      ...
      const descParts = [];
      if (views !== void 0) {
        descParts.push(`\u{1F441}\uFE0F ${viewsText}`);
      }
      const $boxInner = $(".box.info .box-inner-block").clone();
      $boxInner.find("br").replaceWith("\n");
      const cleanInfoText = $boxInner.text().split("\n").map((line) => line.trim()).filter((line) => line && !line.includes("INFORMATION:") && !line.includes("For albums with")).join("\n").replace(/:\s*\n\s*/g, ": ");
      if (cleanInfoText) {
    ...
    parseChapterList($) {
      ...
      const mainSlug = canonicalHref ? canonicalHref.replace(/^https?:\/\/[^\/]+\//, "").replace(/\/$/, "").trim() : "";
      $(".page-link .post-page-numbers").each((_, element) => {
        ...
        if ($el.is("a")) {
          ...
          chapterId = href.replace(/^https?:\/\/[^\/]+\//, "").replace(/\/$/, "").trim();
        } else {
          chapterId = mainSlug;
        }
        if (chapterId) {
      ...
      categoryTitles.forEach((catTitle, index) => {
        ...
        $(".entry p.post-meta").each((_, pEl) => {
          ...
          if ($p.text().toLowerCase().includes(catTitle.toLowerCase())) {
            $p.nextUntil("p.post-meta, .clear", "span.tag-counterz").each((_2, spanEl) => {
              const $span = $(spanEl);
  ...
  // src/Misskon/MisskonSetting.ts
  var DEFAULT_BASE_URL = "https://misskon.com";
  ...
  var Misskon = class {
    ...
    async fetchMangaPageCached(realMangaId) {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      return App.createPagedResults({
        results: manga,
        metadata: hasNextPage ? { page: nextPageNum } : void 0
      });
    }
  ...
  return __toCommonJS(Misskon_exports);
})();
